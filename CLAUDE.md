...
- `src/notes/defaults/`: the packaged data `notes init` installs into the vault: `config.toml`, `gitignore`, `prompt.md` (the shared drafting prompt), `DEFAULTS_VERSION`, and `types/<kind>/{prompt.md,template.md}` for `decision`, `fact`, `reminder`, `idea`. This is the only place prompts and templates live in the repository. Once installed, the copies under `~/.notes/` are the ones the CLI reads, and it never overwrites them; when a packaged prompt or template changes, bump `DEFAULTS_VERSION` so `notes check` can report that newer defaults exist.
- `plugin/`: the agent plugin: `.claude-plugin/plugin.json` (its version equals `notes.__version__`), `hooks/hooks.json` and `hooks/recall.sh` (the `UserPromptSubmit` hook), `skills/{decision,fact,idea,note,recall,reminder}/SKILL.md`. There is one drafting skill per built-in kind, so the kind is settled by which description matched rather than by a choice made after a catch-all skill loaded; `note` covers the kinds a user adds under `~/.notes/types/`, and `recall` reads. A skill's description names the trigger phrases of its kind and points at the sibling skills it competes with; adding a kind means adding a skill. The directory is not called `claude-plugin` because Codex reads this same layout, manifests included; one directory serves both agents, so a second one is never needed.
