...
from notes import clock, queries, schedule
...
def list_command(
    ...
    emit(ctx, render(session.vault, rows, clock.now()), [queries.as_data(session.vault, row) for row in rows])


def render(vault: Path, rows: Iterable[NoteRow], now: datetime) -> str:
    """One line per note: `[unread] <date> <kind> <effective status> [id](abs) - Title (schedule)`, as aligned columns.

    The date is the creation date, the sort key of the listing. The kind and status columns are padded to the widest
    value in the listing. The `[unread]` column exists only when some note is unread; the read notes then carry blanks
    in its place, so the links start at the same column. A scheduled note ends with its schedule as `describe` reads
    it at `now`.
    """
    rows = list(rows)
    ...
    return "\n".join(render_line(vault, row, now, marker_width, kind_width, status_width) for row in rows)


def render_line(
    vault: Path, row: NoteRow, now: datetime, marker_width: int = 0, kind_width: int = 0, status_width: int = 0
) -> str:
    ...
    columns.append(row.created_date)
    columns.append(style_kind(row.kind.ljust(kind_width)))
    columns.append(style_status(row.effective_status.ljust(status_width)))
    detail = schedule.describe(schedule.parse(row.schedule), now) if row.schedule else None
    columns.append(render_link(vault, row.path, row.title, detail))
    return " ".join(columns)
